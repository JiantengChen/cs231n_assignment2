...
class FullyConnectedNet(object):
        ...
        for i in range(1, self.num_layers + 1):
            if i == 1:
                self.params[f"W{i}"] = np.random.normal(
                    0, weight_scale, (input_dim, hidden_dims[i - 1])
                )
                self.params[f"b{i}"] = np.zeros(hidden_dims[i - 1])
            elif i == self.num_layers:
                self.params[f"W{i}"] = np.random.normal(
                    0, weight_scale, (hidden_dims[i - 2], num_classes)
                )
            ...
            if (
                self.normalization in ["batchnorm", "layernorm"]
                and i != self.num_layers
            ):
    ...
    def loss(self, X, y=None):
        ...
        for i in range(self.num_layers):
            ...
            w, b, gamma, beta = (self.params.get(key) for key in keys)
            ...
            if i == self.num_layers - 1:
                ...
                if self.normalization == "batchnorm":
                    X, cache[i] = affine_bn_relu_forward(X, w, b, gamma, beta, bn)
                elif self.normalization == "layernorm":
                    X, cache[i] = affine_ln_relu_forward(X, w, b, gamma, beta, bn)
                else:
                    X, cache[i] = affine_relu_forward(X, w, b)

                if self.use_dropout:
                    X, cache[f"dropout{i+1}"] = dropout_forward(X, dropout)
